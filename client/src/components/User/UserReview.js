...
const UserReview = ({ review, uuid, onDelete, onUpdate, readOnly = false }) => {
  ...
  // Whether the currently logged-in user owns this review
  const isOwner = !readOnly && uuid === review.user_id;

  return (
    <Box mt={2}>
      <Box sx={{ border: '3px solid', borderColor: 'divider', borderRadius: 2, p: 0.5, mb: 0.5 }}>
        ...
        {editingReviewId === review.review_id ? (
          <>
            ...
            <TextField
              ...
              sx={{ mb: 1 }}
            />
            <Button variant="contained" size="small" onClick={() => handleSave(review.review_id)} sx={{ mr: 1 }}>
              Save
            </Button>
          ...
          <>
            <Typography variant="h6" color="text.secondary">
              {review.deal_name} •{' '}
              <Box component="span" sx={{ fontStyle: 'italic' }}>{review.restaurant_name}</Box>
            </Typography>
